...
def draw_cockpit(surface, game_state: GameState) -> Dict[str, List[pygame.Rect]]:
    ...
    for i, (sys_key, sys_name) in enumerate(systems.items()):
        ...
        plus_rect = pygame.Rect(x_pos + 160, 100, 40, 100)
        minus_rect = pygame.Rect(x_pos + 160, 210, 40, 70)
    ...
    for i, (sys_key, sys_name) in enumerate(nav_systems.items()):
        ...
        value = getattr(game_state, sys_key)
        # ... (Drawing logic for nav systems) ...
        
    draw_text(surface, f"Energie: {game_state.energy_pool}", (cockpit_rect.centerx, 600), center=True, f=big_font)
